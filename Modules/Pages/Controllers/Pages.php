...
// ✅ Tell PHP which PHPMailer classes we want to use
...
class Pages extends FrontendController {
    ...
    public function financing(){
        ...
        $data['meta_desc']		= $pg_data->meta_desc;
        $data['meta_keyword']	= $pg_data->meta_keyword;
        $data['page_title']	    = $pg_data->page_title ?: 'Think Outside The Bank';
        $data['page_content']	= str_replace(
            ['{{img_path}}', '{{base_url}}'],
            [base_url().'/public/assets/', site_url('/')],
            $pg_data->page_content
        );
        $data['mylib'] = $this->mylib;
        $data['include']        = $this->viewDirectory . '\financing_views';
    ...
    */
    ...
    public function contact_us(){
        ...
        $data['meta_title'] = (isset($meta_info->meta_title) && !empty($meta_info->meta_title))?$meta_info->meta_title:'Contact Us | Sirius Motorsports Inc. | Tillsonburg, Ontario';
        $data['meta_desc'] = (isset($meta_info->meta_desc) && !empty($meta_info->meta_desc))?$meta_info->meta_desc:'Contact Sirius Motorsports Inc. in Tillsonburg, Ontario. Call 519-635-8872 or email harley@siriusconinc.com.';
        $data['meta_keyword'] = (isset($meta_info->meta_keyword) && !empty($meta_info->meta_keyword))?$meta_info->meta_keyword:'Contact Sirius Motorsports, Tillsonburg Used Cars';
    ...
    public function post_contact_us(){
        ...
        if ($this->request->isAJAX()) {
            ...
            $enquiry_type  = trim((string) $this->request->getPost('enquiry_type'));
            if ($enquiry_type !== 'interest') {
            ...
            if ($insert_id) {
                ...
                if ($is_send_mail) {
                    ...
                    $mailer->setFrom('info@internationalphotoworkshops.com', 'International Photo Workshops');
                   ...
                   // $mailer->setTo($siteinfo->user_email);                     // Admin recipient
                    $mailer->setTo('manuello@manuellopaganelli.com');                     // Admin recipient
                   // $mailer->setReplyTo($email_addr, $name);                   // Let admin reply to the visitor safely
                    ...
                    if (filter_var($email_addr, FILTER_VALIDATE_EMAIL)) {
                        $mailer->setReplyTo($email_addr, $name);
                    }
    ...
    public function post_contact_usPrevious()
    {
        $mylib = new MyLibrary();
        $siteinfo = $mylib->siteinfo();
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($insert_id) {
                ...
                if ($is_send_mail) {
                    ...
                    $mailer->setFrom($cfg->fromEmail, $cfg->fromName);         // Zoho-authenticated sender (do not use visitor here)
                   // $mailer->setTo($siteinfo->user_email);                     // Admin recipient
                    $mailer->setTo('info@internationalphotoworkshops.com');                     // Admin recipient
                    $mailer->setReplyTo($email_addr, $name);                   // Let admin reply to the visitor safely
                    $mailer->setSubject($subjectAdmin);
                    $mailer->setMessage($bodyAdmin);
                    $mailer->setMailType('html');
    ...
    public function port_test(){
        ...
        try {
            /** -----------------------------
             *  OPTION 1: Use PHP mail() (no SMTP)
             * ----------------------------- */
            $mail->isMail(); // use the built-in mail() function

            // Sender and recipient
            $mail->setFrom('info@internationalphotoworkshops.com', 'International Photo Workshops');
            $mail->addAddress('developer76076@gmail.com', 'Sumit');

            // Content
            $mail->isHTML(true);
            $mail->Subject = 'Test Email via PHPMailer (no composer)';
            $mail->Body    = '<h3>Hello Boss 👋</h3><p>This email was sent using PHPMailer without Composer!</p>';
            $mail->AltBody = 'Hello Boss! This is a plain text version.';
